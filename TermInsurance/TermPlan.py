...
profile_agent = initialize_agent(
    tools=[profile_tool],
    llm=llm,
...
def fetch_best_term_plans(_: str) -> str:
    try:
        ...
        query = "current best 1cr term insurance plans in India"
...
term_plan_tool = Tool(
    ...
    description="Fetches current best 1cr term insurance plans using SerpAPI."
)

term_plan_agent = initialize_agent(
    tools=[term_plan_tool],
    llm=llm,
    agent="zero-shot-react-description",
    verbose=True
)
...
def manager():
    ...
    if "Eligible" in validation_result:
        plans = term_plan_agent.run("Fetch best 1cr term insurance plans providers names")
        logging.info(f"Best 1cr Term Plans: {plans}")
        subject = "Your 1cr Term Insurance Eligibility & Best Plans"
        body = f"Congratulations! You are eligible for 1cr term insurance. Here are the best plans:\n\n{plans}"
    else:
